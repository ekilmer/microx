...
// The x86 / x86-64 micro-execution backend. It decodes with Intel XED and
// executes an instruction natively via an inline-assembly harness that shuttles
// the guest register/flag/FPU state through the real machine registers. All of
// this only compiles on an x86-64 host; the CMake build selects this TU only
// for x86-family targets.
...
static bool ReadAllMemory(const Executor* executor) {
...
static void CopyMMXStateToFPU(size_t addr_size) {
  if (32 == addr_size) {
    COPY_FROM_MMX_32(0);
    COPY_FROM_MMX_32(1);
    COPY_FROM_MMX_32(2);
...
static void CopyMMXStateFromFPU(size_t addr_size) {
  if (32 == addr_size) {
    COPY_TO_MMX_32(0);
    COPY_TO_MMX_32(1);
    COPY_TO_MMX_32(2);
...
static void LoadFPU(size_t addr_size) {
  if (!gUsesFPU) return;
  if (32 == addr_size) {
...
static void StoreFPU(size_t addr_size) {
  if (!gUsesFPU) return;
  if (32 == addr_size) {
    asm("fxsave %0;"
        ".byte 0x48; fxrstor %1;"
...
// AVX / AVX-512 native execution is not implemented. Raising `SIGILL` routes
// through the driver's fault handler, yielding `kErrorExecute`, matching the
// original behavior of reporting an illegal instruction.
static void ExecuteNativeAVX(void) {
  raise(SIGILL);  // TODO(pag): Implement this!
}
...
// The x86 / x86-64 backend. Each method is a thin adapter over the free
// functions above; the intricate decode/emulate/execute logic is unchanged
// from the original single-file implementation.
class X86Backend final : public Backend {
 public:
  X86Backend(size_t addr_size, bool has_avx, bool has_avx512)
      : addr_size_(addr_size), has_avx_(has_avx), has_avx512_(has_avx512) {}

  ~X86Backend(void) override {}
  ...
  ExecutorStatus FetchDecode(const Executor* executor, uintptr_t pc) override {
    const auto fetch_pc = executor->ComputeAddress(
        "CS", pc, 0, 0, 0, 8, MemRequestHint::kReadExecutable);

    // The maximum possible instruction length given our memory model. Probe
    // downward so a fetch that would run off the end of mapped memory still
    // finds the largest readable window. A failed probe is expected here (the
    // binding treats an executable-fetch `ReadMem` failure as non-fatal), so
    // we simply keep shrinking.
    Data idata;
    size_t inst_length = 15;
    for (; inst_length; --inst_length) {
      if (executor->ReadMem(fetch_pc, inst_length * 8,
    ...
    if (!DecodeInstruction(idata.bytes, inst_length, addr_size_)) {
  ...
  ExecutorStatus CheckSupported(const Executor* executor) override {
    return UsesUnsupportedFeatures(executor)
               ? ExecutorStatus::kErrorUnsupportedFeatures
               : ExecutorStatus::kGood;
  }

  ExecutorStatus ReadInputs(const Executor* executor) override {
    // Flags and registers, treated uniformly as named registers.
    if (!ReadFlags(executor)) {
      return ExecutorStatus::kErrorReadFlags;
    }
    if (!ReadRegisters(executor)) {
      return ExecutorStatus::kErrorReadReg;
    }

    // The FPU / MMX state is only touched when the instruction actually uses
    // it, which never happens for a NOP, so reading it here (rather than after
    // the NOP short-circuit) is equivalent to the original ordering.
    if (gUsesFPU && !executor->ReadFPU(gFPU)) {
      return ExecutorStatus::kErrorReadFPU;
    }
    if (gUsesMMX) {
      CopyMMXStateToFPU(addr_size_);
    }
    return ExecutorStatus::kGood;
  }

  bool IsNoOp(void) const override {
    const auto category = xed_decoded_inst_get_category(gXedd);
    return XED_CATEGORY_NOP == category || XED_CATEGORY_WIDENOP == category;
  }

  ExecutorStatus ReadMemory(const Executor* executor) override {
    return ReadAllMemory(executor) ? ExecutorStatus::kGood
                                   : ExecutorStatus::kErrorReadMem;
  }

  Action ComputeControlFlow(const Executor* executor, uintptr_t& next_pc,
                            ExecutorStatus& status) override {
    next_pc = GetNextPC(executor);
    if (IsNoOp()) {
      return Action::kEmulated;
    }

    ExecutorStatus emulation_status = ExecutorStatus::kGood;
    if (Emulate(executor, next_pc, emulation_status)) {
      return Action::kEmulated;
    }
  ...
  bool EncodeNative(const Executor* executor) override {
    return EncodeInstruction(executor);
  }

  void RunNative(void) override {
    LoadFPU(addr_size_);
    if (has_avx512_) {
      ExecuteNativeAVX512();
    } else if (has_avx_) {
  ...
  ExecutorStatus WriteOutputs(const Executor* executor,
    ...
    if (!WriteMemory(executor)) {
      return ExecutorStatus::kErrorWriteMem;
    }
    if (gUsesFPU && !executor->WriteFPU(gFPU)) {
      return ExecutorStatus::kErrorWriteFPU;
    }
    if (gUsesMMX) {
      CopyMMXStateFromFPU(addr_size_);
    }
    if (!WriteFlags(executor)) {
      return ExecutorStatus::kErrorWriteFlags;
    }

    // Write back any registers that were read or written, program counter last.
    SetNextPC(executor, next_pc);
    if (!WriteRegisters(executor)) {
...
bool Backend::GlobalInit(void) {
  ...
  if (MAP_FAILED == ret || gExecArea != ret) {
    gExecArea = nullptr;
    return false;
  }
...
Backend* Backend::Create(Arch arch, size_t addr_size, bool has_avx,
                         bool has_avx512) {
  if (Arch::kX86 == arch || Arch::kAMD64 == arch) {
    return new (std::nothrow) X86Backend(addr_size, has_avx, has_avx512);
  }
  return nullptr;
}
